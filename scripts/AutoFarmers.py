...
FREE_SOFTWARE_MESSAGE = """=====================================================================
  ...
  Entra a la pelea y pulsa START: el bot actuara cuando sea tu turno.
=====================================================================
...
# Informacion mostrada en la GUI para la Estrategia Canopus.
REQUIREMENTS = {
    "Estrategia Canopus": """
...
<p><em>Y como decía mi abuelo: entre más pequeña más aprieta. CHA CHAU</em></p>
<p>Entra a la pelea ANTES de pulsar START; el bot actúa solo en tu turno.</p>
    """,
}
...
FARMER_IMAGES = {
    "Estrategia Canopus": "dk_farmer.jpg",
}
...
def get_farmer_display_content(farmer_name: str, whale_enabled: bool = False) -> tuple[str, str]:
    ...
    return farmer_name, FARMER_IMAGES.get(farmer_name)


# Farmer script definitions (argument structure)
# "Estrategia Canopus" no usa la capa de farmer: su script arranca directamente
# la arquitectura fighter + strategy dentro de la pelea.
CANOPUS_FARMER = {
    "name": "Estrategia Canopus",
    "script": "EstrategiaCanopus.py",
    "args": [],
}
FARMERS = [CANOPUS_FARMER]
...
# La Estrategia Canopus no usa --password.
...
class FarmerTab(QWidget):
    ...
    def _build_left_panel(self):
        ...
        if self.farmer["name"] in REQUIREMENTS or self.farmer["name"] in WHALE_MODE_CONFIG:
            ...
            # Sin scroll: el cuadro crece hasta mostrar todo el texto.
...
class ListView(QWidget):
    ...
    def __init__(self, farmers, controllers, parent=None):
        ...
        self._row_to_farmer: dict = {}  # stack/row index → farmer name
        self._init_ui()
    ...
    def _init_ui(self):
        ...
        # Filas: solo los farmers
        for i, farmer in enumerate(self._farmers):
            item = QListWidgetItem("  " + farmer["name"])
            self._list.addItem(item)
            self._list_rows[farmer["name"]] = i
            self._row_to_farmer[i] = farmer["name"]
        ...
        # Un placeholder por fila; el widget real se construye al seleccionarla.
        for _ in range(len(self._farmers)):
        ...
        # Seleccionar la primera fila; el contenido se construye en el siguiente tick.
        initial_row = 0
        self._list.blockSignals(True)
        self._list.setCurrentRow(initial_row)
        self._list.blockSignals(False)
        QTimer.singleShot(0, lambda: self._on_row_changed(initial_row))
